<template>
    ...
    <div
        ...
        @click="close()"
    >
        <Card class="bg-card shadow-lg rounded-lg overflow-hidden" @click.stop>
            <CardContent class="max-h-[80vh] overflow-y-auto">
                <draggable
                    v-model="editedColumns"
                    :move="onMove"
                    handle=".handle"
                    item-key="name"
                    class="space-y-2"
                        ...
                        <div
                            ...
                            :class="{
                                'bg-muted opacity-70 cursor-not-allowed':
                                    isLocked(element.name),
                                'bg-background': !isLocked(element.name),
                            }"
                        >
                            <span
                                class="handle cursor-move text-muted-foreground"
                                >☰</span
                            >
...
export default {
    ...
    methods: {
        ...
        onMove(evt) {
            ...
            if (this.isLocked(dragged.name) || this.isLocked(target?.name))
                return false;
            return true;
        },
        isLocked(name) {
