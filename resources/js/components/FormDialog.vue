        ...
        <div class="space-y-4">
          <template v-for="(field) in fields">
              <div v-if="['text','email','phone'].includes(field.type)" :key="field.name">
                <label class="block text-sm font-medium text-gray-700">{{ field.label }} <span v-if="field.required" class="text-red-800">*</span></label>
                <input :key="field.name" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm  text-gray-900 placeholder-gray-400 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" v-model="form[field.name]" :type="field.type" :name="field.name" :id="field.id" :hint="field.hint" :required="field.required"/>
              </div>
          </template>
        </div>
