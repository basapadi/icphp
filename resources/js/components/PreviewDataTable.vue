<template>
    <div class="relative" ref="tableContainer">
        <table
            class="table-auto border-collapse border border-dashed border-primary/20 z-1"
        >
            <thead class="bg-primary/10 sticky top-0" style="z-index: 11">
                <tr>
                    <template v-for="column in columns" :key="column.value">
                        <template v-if="column.show">
                            <th
                                :class="`px-4 py-2 text-left font-bold text-xs text-muted-foreground uppercase border border-1 border-dashed border-border text-${column.align}`"
                                :style="`${column.styles}`"
                            >
                                {{ column.label }}
                            </th>
                ...
                <tr
                    ...
                    :class="[
                        ...
                        selectedIndex.includes(index)
                            ? 'bg-primary/20'
                            : 'hover:bg-primary/10 odd:bg-muted even:bg-background',
                    ]"
                >
                    <template v-for="column in columns" :key="column.value">
                        <td
                        ...
                        >
                            <template v-if="column.name == 'actions'">
                                <button
                                    @click.stop="
                                        toggleDropdown(column, data, $event)
                                    "
